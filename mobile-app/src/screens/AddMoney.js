...
export default function AddMoneyScreen(props) {
    ...
    const [state, setState] = useState({
        ...
        amount: '5',
        qickMoney: [{amount: '5', selected: false}, {amount: '10', selected: false}, {
            amount: '20',
            selected: false
        }, {amount: '50', selected: false}, {amount: '100', selected: false}],
    });
    ...
    const newData = ({item, index}) => {
        return (
            <TouchableOpacity
                style={[styles.boxView, {backgroundColor: item.selected ? colors.GREY.default : colors.GREY.primary}]}
                onPress={() => {
                    quckAdd(index);
                }}>
                <Text style={{
                    ...styles.quckMoneyText,
                    color: item.selected ? colors.WHITE : colors.BLACK
                }}>{settings.symbol}{item.amount}</Text></TouchableOpacity>
        )
    }
    ...
    return (
        <View style={styles.mainView}>
            ...
            <View style={styles.bodyContainer}>
                <Text style={styles.walletbalText}>{language.Balance} - <Text
                    style={styles.ballance}>{settings.symbol}{state.userdata ? parseFloat(state.userdata.walletBalance).toFixed(2) : ''}</Text></Text>
