  ...
  return (
    <View style={styles.mainView}>
      <Header
        ...
        rightComponent={ userdata && userdata.usertype == 'rider' && booking.status =='PAYMENT_PENDING'?<TouchableOpacity onPress={cancelCurBooking}><Text style={{color:colors.WHITE}}>{language.cancel}</Text></TouchableOpacity>:null}
        containerStyle={styles.headerStyle}
        innerContainerStyles={{ marginLeft: 10, marginRight: 10 }}
      />
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollStyle}>
        <View style={{ flex: 1, flexDirection: 'column' }}>
          <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 20, paddingRight: 20, marginBottom: 4 }}>
            ...
            {userdata && userdata.usertype == 'rider' && booking.status == 'PAYMENT_PENDING'? 
          ...
          {userdata ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: colors.BLACK, textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{userdata.usertype == 'rider' ? language.your_fare: language.total_fare}</Text>
          ...
          {userdata ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: colors.GREEN.bright, textAlign: 'left', lineHeight: 45, fontSize: 24, fontWeight: '500' }}>{language.payable_ammount}</Text>
              <Text style={{ color: colors.GREEN.bright, textAlign: 'left', lineHeight: 45, fontSize: 24, fontWeight: 'bold' }}>{settings.symbol} {payDetails.payableAmmount ? parseFloat(payDetails.payableAmmount).toFixed(2) : 0.00}</Text>
            </View>
            : null}
        </View>
        {payDetails.payableAmmount == 0 ?
          <View style={styles.buttonContainer}>
            <TouchableOpacity
              ...
              <Text style={styles.buttonTitle}>{userdata && userdata.usertype == 'rider'?language.paynow_button: language.ok}</Text>
            </TouchableOpacity>
          </View>
          :
          <View style={styles.buttonContainer}>
